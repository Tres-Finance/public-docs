# API example: gather sub txs by identifier

Runnable example script `gather_sub_txs_by_identifier.py` from the public-docs repo (api-examples/).
...
def get_sub_txs(graphql_client: GraphQLClient, tx_identifiers: list[str]) -> list[SubTransaction]:
    res =  get_sub_transactions_data_in_account_context(graphql_client=graphql_client,
        tx_identifiers=tx_identifiers, limit=10000
    )
    return res
